...
async def startup_event():
    logger.info("Application starting up", extra={"extra_fields": {"version": "1.0.0"}})
...
async def healthz():
    health_status = {
        ...
        "service": "startup-compliance-health-check-api",
        "version": "1.0.0",
        "checks": {}
    }
    ...
    try:
        ...
        health_status["checks"]["database"] = {
            "status": "healthy",
            "type": "postgresql" if os.getenv("DATABASE_URL") else "in-memory"
        }
    ...
    try:
        email_service_status = "configured" if (
            os.getenv("AWS_ACCESS_KEY_ID") or os.getenv("SMTP_HOST")
        ) else "not-configured"
        health_status["checks"]["email_service"] = {
            "status": "healthy",
            "configured": email_service_status
        }
    except Exception as e:
        health_status["checks"]["email_service"] = {
            "status": "unhealthy",
...
async def compute_assessment(submission: AssessmentSubmission):
    try:
        logger.info("Processing assessment", extra={"extra_fields": {
        ...
        logger.info("Assessment completed", extra={"extra_fields": {
        ...
        try:
            ...
            logger.info("Email notification sent successfully", extra={"extra_fields": {
                "assessment_id": result.id
            }})
        except Exception as email_error:
            logger.error("Failed to send email notification", extra={"extra_fields": {
                "assessment_id": result.id,
